...
export function InboxSender({ handle, publicKey }: { handle: string; publicKey: string }) {
  ...
  const send = useCallback(async () => {
    ...
    try {
      ...
      const stamp = await sealToInbox(publicKey, body);
    ...
    return (
      <div className="border border-hairline p-10 text-center">
        ...
        <div className="mt-7 flex flex-wrap justify-center gap-3">
          <button type="button" onClick={() => setSent(false)} className="cmd">
            Send another
          </button>
          <Link href="/inbox" className="cmd-primary">
  ...
  return (
    <div className="space-y-5">
      <header className="text-center">
        <div className="stamp mx-auto mb-5">End-to-end encrypted</div>
        <h1 className="text-balance text-3xl font-semibold tracking-tight sm:text-4xl">
          Tell <span className="text-amber">/{handle}</span> what you really think
        </h1>
      ...
      <div className="border border-hairline p-5 sm:p-6">
        <textarea
          ...
          autoFocus
          className="input min-h-[160px] resize-none text-[17px] leading-relaxed"
          aria-label="Your anonymous message"
        />
        <div className="mt-2 flex items-center justify-between text-xs">
          <span className="text-label">Be honest. Do not be cruel.</span>
          <span className={remaining < 0 ? 'text-alert' : 'text-label'}>{remaining}</span>
        </div>
      </div>
      ...
      <button type="button" onClick={send} disabled={!canSend} className="cmd-primary w-full">
